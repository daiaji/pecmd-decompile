    ...
    *plen = 0;
    ...
    if (q == L'"' || ((flags & 0x40) != 0 && q == L'\'')) {
        ...
        do {
            ...
            (((flags & 0x400) != 0 && *cur != 0) && (c = cur[1], c != 0) &&
             ((c < 9 || c > 0xd) && c != 0x20)) ||
            /* 0x200: 引号交替转义, 奇数层或紧跟非空白 -> 继续 */
            (((flags & 0x200) != 0 && *cur != 0) &&
             (c = cur[1],
              c != 0 && (flip = (bool)(flip ^ 1), flip || ((c < 9 || c > 0xd) && c != 0x20)))));

        /* 闭合引号后紧跟非空白 -> 扩展到空白 */
        if (*cur == q && (flags & 0x80) == 0 &&
            ((flags & 0x10) == 0 && (cur[1] != 0 && (cur[1] < 9 || cur[1] > 0xd)) &&
             cur[1] != 0x20)) {
            cur++;
 ...
 * 构造 "~S/服务 参数" 或 "~I/Service ..." 命令文本交给 PECMD_RunBootScriptInFiber
 * (PECMD_ServiceEntry) */
