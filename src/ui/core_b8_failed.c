 ...
 *       真实语义已由反汇编注释保留。 ---- */
BOOL CryptEnumProviderTypesW(DWORD dwIndex, DWORD *pdwReserved, DWORD dwFlags, DWORD *pdwProvType,
                             DWORD cbProvType, WCHAR *pszProvTypeName)
{
    ...
    (void)pdwProvType;
    (void)cbProvType;
    (void)pszProvTypeName;
    return FALSE;
}
BOOL CryptGetProvParam(HCRYPTPROV hProv, DWORD dwParam, BYTE *pbData, DWORD *pdwDataLen,
                       DWORD dwFlags)
{
    (void)hProv;
    (void)dwParam;
...
extern int64_t *PECMD_StrCopyA(int64_t *ps, LPCSTR src,
                               int64_t len);            /* @0x1400634d4 ANSI 串写入容器 */
...
DWORD PECMD_CryptoHashCompute(BYTE *data, DWORD len, ALG_ID alg, uint64_t extra)
{
    ...
    if ((DWORD)alg == 0xffffcfc7u) {
...

        {
            DWORD dwIndex = 0;
            for (;;) {
                if (CryptEnumProviderTypesW(dwIndex, NULL, 0, &dwA, 0, (WCHAR *)&subLen) == FALSE) {
                    break;
                }
                CryptEnumProviderTypesW(dwIndex, NULL, 0, &dwA, (DWORD)(uintptr_t)ebp,
                                        (WCHAR *)&subLen);
                dwIndex++;
            }
        }
        ...
        do {
            dwC = 0;
            if (CryptGetProvParam(*(HCRYPTPROV *)&pE, 1 /* PP_ENUMALGS */, r15, &dwB, (DWORD)r13) ==
                FALSE) {
                esi = 0;
            }
            else {
                ...
                switch (v0 & 0xe000) {
                case 0x2000:
                    label = "\xC7\xA9\xC3\xFB";
                    break; /* 签名 @0x14012b104 */
                case 0x6000:
                    label = "\xBC\xD3\xC3\xDC";
                    break; /* 加密 @0x14012b10c */
                case 0x8000:
                    label = "\xB9\xFE\xCF\xA3";
                    break; /* 哈希 @0x14012b114 */
                case 0xa000:
                    label = "\xBD\xBB\xBB\xBB";
                    break; /* 交换 @0x14012b11c */
                default:
                    label = "\xCE\xB4\xD6\xAA";
                    break; /* 未知 @0x14012b124 */
                }

                wsprintfA(pC, "%8.8xh  %4d  %4s  %4d  %s\r\n", (int32_t)v0, (int32_t)v1, label,
                          (int32_t)v2, pD);        /* fmt @0x14012b0e8 */
...
extern uint16_t FUN_1400F172C(int64_t *map, int msg, uint64_t wParam, uint64_t *lParam,
                              int64_t hwnd, uint32_t mode, int64_t *out); /* @0x1400f172c */
...
extern int64_t FUN_1400E5B0C(int64_t a1, uint64_t a2, int64_t a3, int64_t *a4); /* @0x1400e5b0c */
...
void PECMD_HandleControlMessage(void *param_1, int param_2, int64_t param_3, uint64_t param_4)
{
    ...
    if (msg == 0x44f) {
        ...
        if (PtInRect(&rc, pt) != 0) {
            ...
            FUN_1400F172C(*(int64_t **)(obj + 0xd8), ebp, 0, NULL, (int64_t)hwnd, 0x80, NULL);
            return;
        }
        else {
            if ((int8_t)obj[0xad] <= 0) {
            ...
            if ((obj[0xd1] & 0x40) == 0) {
                ...
                ebx |= 0x2020000;
                (void)GetParent(hwnd);
                PostMessageW((HWND)(intptr_t)GetParent(hwnd), 0x441, (WPARAM)ebx, (LPARAM)0x202);
                ebp = 0x202;
                PECMD_RestoreForegroundWindow();
                FUN_1400F172C(*(int64_t **)(obj + 0xd8), ebp, 0, NULL, (int64_t)hwnd, 0x80, NULL);
            }
    ...
    if (msg == 0x44e) {
        /* 虚表方法 +0xe0 转发 */
        {
            int64_t (*fn)(void *, int64_t) =
                (int64_t (*)(void *, int64_t))(uintptr_t)(*(uint64_t *)((uint8_t *)param_1 + 0xe0));
    ...
    if (msg == 0x45a) {
        esi = (((uint8_t)obj[0xa4] & 0x10) == 0 && ((uint8_t)obj[0xa4] & 0x3) != 0) ? 1 : 0;
        PECMD_SetControlText(*(HWND *)(obj + 0x20), wParam, (LPCWSTR)lParam, esi);
        ebx = 1;
        return;
    }
    ...
    /* ===== 默认分支：字段追加 ===== */
    {
        ...
        if (count > 0) {
            sub40 = 0;
            rv = FUN_1400F172C(map, r13d, (uint64_t)wParam, (uint64_t *)&lParam, (int64_t)hwnd, 1,
                               (int64_t *)&sub40);
            if (rv & 0x4) {
                ebx = (int32_t)sub40;
            }
            else {
                ebx = (int32_t)FUN_1400E5B0C((int64_t)param_1, (uint64_t)r13d, wParam,
                                             (int64_t *)&lParam);
                if (count > 0) {
                    FUN_1400F172C(map, r13d, (uint64_t)wParam, (uint64_t *)&lParam, (int64_t)hwnd,
                                  2, NULL);
                }
            }
        }
        ...
        if (r12 != 0) {
            PostMessageW((HWND)r12, 0x451, (WPARAM)0x204, (LPARAM)0xffffffffaa555aa5ULL);
        }
        (void)ebx;
        return;
    }
