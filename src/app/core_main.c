...
WCHAR *FUN_1400637DC(WCHAR **ps, LPCSTR src, int64_t srclen,
                     int64_t codepage); /* @0x1400637dc core_init.c */
...
static int PECMD_MainW(HINSTANCE hInstance, WCHAR *cmdline)
{
    ...
    for (;;) {
        ...
        /* MAIN 命令: 执行脚本 */
        if (FUN_14005C788("MAIN", pToken, 4) && ((pMain = pToken + 4, *pMain > 8 && *pMain < 0xe) ||
                                                 (*pMain == L' ' || *pMain == L'*'))) {
