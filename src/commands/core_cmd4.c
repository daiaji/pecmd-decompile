...
static inline int CodeWs(WCHAR c)
{
    return ((8 < (uint16_t)c) && ((uint16_t)c < 0xe)) || (c == 0x20);
}
        ...
        if (srcStar == 0) {
                ...
                if (bytesRead >= 0) {
                    ...
                    if (srcBom == 0 && srcType != 0x4b0) {
                        if (srcType == 0x4b1) {
                            if (FUN_14005B184((const char *)data, (const char *)BOM_BE, 2) == 0) {
                                ...
                                if (srcBom == 0 && srcCp == 0xfde9) {
                                    if (FUN_14005B184((const char *)data, (const char *)BOM_UTF8,
                                                      3) == 0) {
                                        skip = 3;
                                        srcCp = 0xfde9;
                                    }
                        ...
                        if (FUN_14005B184((const char *)data, (const char *)BOM_LE, 2) == 0) {
                            ...
                            if (srcType == 0x4b1) {
                                if (FUN_14005B184((const char *)data, (const char *)BOM_BE, 2) ==
                                    0) {
                                    skip = 2;
                                    srcType = 0x4b1;
                                }
                                    ...
                                    if (srcBom == 0 && srcCp == 0xfde9 &&
                                        FUN_14005B184((const char *)data, (const char *)BOM_UTF8,
                                                      3) == 0) {
                                        skip = 3;
                                        srcCp = 0xfde9;
                                    }
                                     ...
                                     FUN_14005B184((const char *)data, (const char *)BOM_UTF8, 3) ==
                                         0) {
                                skip = 3;
                                srcCp = 0xfde9;
                            }
            ...
            L_var_path: {
                ...
                if (node == NULL) {
                    ...
                    datalen = (int64_t)(*(uint64_t *)((uint8_t *)node + 0x18) & 0x3fffffffffffffff);
                    if (skipCount == 3) {
            ...
            L_data_ok:
                ...
                int64_t n;
                WCHAR *q = src;
                if (!PECMD_ParseNumSkipWs(&q, &n) || *q != 0)
                    goto L_var_path;
                datalen = n;
                if (skipCount == 3) {
                    PECMD_StrBldCopyWide((WCHAR **)&data, sArg2);
    ...
    L_output: /* LAB_14008a38e / LAB_14008a1a8 入口 */
    {
        ...
        nSmall = (int32_t)(datalen / 2);
        {
            ...
            if (srcType == 0) {
                ...
                wv = sStr;
                r = (*g_pConvFunc)((uint32_t)srcCp, 0, (LPCSTR)data, (int)datalen, sStr,
                                   (int)datalen + 10);
                if (r < 0)
                    r = 0;
                datalen = (int64_t)r * 2;
                    ...
                    PECMD_GrowByteBuffer((void **)&sStr2, cap + 0xb);
                    {
                        ...
                        if (dstStar != 0) {
                            PECMD_VarWriteLine(script, sArg4, (LPCWSTR)mb,
                                               (int64_t)lstrlenA((LPCSTR)mb), (int64_t *)&sArg4,
                                               '\0');
                            goto L_ab5e;
                        }
            ...
            if (node != NULL) {
                ...
                if (srcData != NULL && srcLen > 0) {
                    ...
                    for (i = 0; i < count; i++) {
                        ...
                        if (key != NULL)
                            memcpy(&b, key, (size_t)elem);
                        else
                            b = (uint64_t)(uint8_t)
                                flags; /* 无第二变量时用 flags 低字节作立即数 TODO(verify) */
                        r = a;
                        switch (opMode) {
                        case 0:
                            r = a + b;
                        ...
                        case 2:
                            r = (a >> 1) | (a << (elem * 8 - 1));
                            break; /* 循环右移1位 TODO(verify) */
                        case 3:
                            r = b;
                            break;
                        case 4:
                            acc += (int64_t)(int32_t)(uint32_t)b;
                            break;
                        case 5:
                            acc *= (int64_t)(int32_t)(uint32_t)b;
                            break;
