...
## 6. 收口台账（round 26 补：FUN_ 残局）
| 项 | 值 |
|---|---|
| build/link | **真全绿**（P0 修正 link 误判，补 7 缺失符号桩） |
| rename_map | 802 |
| FUN_ 唯一（core+link_stubs） | ~558 |
| ├ core 简化桩（decompile-failed/未实现） | ~194 |
| ├ link_stubs 定义桩 | ~319（含 ~139 CRT 区，多未调用） |
| ├ 漏改名 | 0（P1 已清） |
| DAT_ 真实引用 | 18（硬骨头，登记） |
| 既定路线 | P0止血✅→P1漏改名✅→P2未实现标注(暂缓)→P3顺序还原→P4命名→P5 DAT_登记 |
| 现实边界 | 真未实现桩静态不可凭空还原，需原二进制/反汇编；可还原者→真代码，不可者→明确标注 |
