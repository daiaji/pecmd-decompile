...
## 6. 收口台账（round 25 最终采数）
| 指标 | 值 |
|---|---|
| core 文件 | 95 |
| rename_map 已命名 PECMD_ | 802 |
| 唯一 FUN_ 引用剩余 | 551（全部已分类：CRT / STUB / DECOMPILE_FAILED / AMBIGUOUS / THUNK，见 residue_classification.json + REVIEW） |
| 真实 DAT_ 剩余 | 17（AMBIGUOUS / 字节重叠 / 无大小数组，需运行时深挖） |
| 编译警告 | ~79（全行为相关，0 机械残留） |
| 构建/链接 | build exit 0 · link 无 undefined/multiple |
| git 提交 | 35 |
| 阶段0-3 | ✅ 完成 |
| 阶段4 | ✅ 命名 802（含深挖新命名 12）；DAT_ 迁 113→17；FUN_ 551 已全部分类登记 |
| 阶段5 | 🟡 机械零残留，剩 ~79 行为相关警告登记 |
| 阶段6 | 🟡 深挖 DAT_/fn-ptr ~96；剩 17 DAT_ + 551(已分类) 待运行时深挖 |
| 阶段7(人类手写风打磨) | 🟡 子任务1(参数局部名)✅ / 子任务3(去解译残留)b3i/b3k/b3_remaining/b1/b2f 已清 /
  子任务2(偏移宏)仅建头未广泛应用 / 子任务4(巨函数拆分)/子任务5(去冗余合并) 未做 |
| 完成判定 | 主干(0-5可行部分)完成且全绿；阶段6/7 已推进、残留为需运行时/逆汇编的非静态可清项，按"不改行为"登记 |
