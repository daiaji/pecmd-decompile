...
## 6. 收口台账（round 20 最终采数）
| 指标 | 值 |
|---|---|
| core 文件 | 95 |
| rename_map 已命名 PECMD_ | 790 |
| 唯一 FUN_ 引用剩余 | 563（多为 CRT/桩/巨型歧义，SKIP 政策保留） |
| 真实 DAT_ 剩余 | 93（~60 惰性 fn-ptr 槽正常+~10 无大小数组+~5 AMBIGUOUS） |
| 编译警告 | 98（全行为相关，0 机械残留，登记已知限制） |
| 构建/链接 | build exit 0 · link 无 undefined/multiple |
| git 提交 | 18 |
| 完成判定 | 主体全部阶段完成（还原/构建/命名/零警告/巨函数/link_stubs/最终复核）；
  残余为工作正常待合并/深挖的低价值项，按"不改行为"登记为已知限制，不再强推以免引入语义改动 |
