...
- **GUI 启动禁令(2026-08-23 用户指令)**: 绝对禁止代理编写/运行可能弹出 GUI 等待点击的程序
  (含 Wine 跑原版 EXE 的冒烟——实测 PECMD 会拉起多进程+尝试窗口消息过滤, 无法保证无人值守)。
  tools/wine_smoke.sh 保留为【用户手跑工具】并在文件头注明; 代理侧验证一律走静态/编译链接门。
  本次单次观测记录: exit=0 且 done.txt 曾=OK ⇒ 解析→ENVI→%展开%→WRITE 主链路在 Wine 下至少跑通过一次;
  IFEX/HASH/REGI 四项 FAIL 待用户手跑复核(INI 语法猜测或 LOAD 多进程时序问题)。
- **网络瞬断处置(2026-08-23)**: 子代理报 failed 且 closing message 为空 = 上游节点网络错误(司空见惯),
  标准动作=立即 send_message 唤醒续作(附当前文件状态+剩余步骤), 而非主代理接手或重派。
  仅当续作再次失败且确认非网络原因时才考虑接手/重派/登记。注意: 接管前先 git diff 检查是否只是
  并发 WIP 造成的一时红态(勿误判), 且 git add -A 会扫入他人半成品——提交前必须核对暂存清单。
...
- **G4-G8 连环命名批完成(git 9918103→d668c4a)**: P1(7)+G2(4+归并)+G3(24,EXEC巨函定名
  ExecCmdDispatch)+G4(24)+G5(40)+G6(61)+G7(70)+G8(13) ≈ **+321名**, rename_map 1157→**1478**,
  未命名 630→309(biz162/crt147)。管线两形态: 归一式(G4前段)/最小干预式(G8起,不碰声明靠原生extern)。
- **在途批次(round20 时点)**: R2空桩还原6个(独占ls) / c8a·c8b·c8c提名121个 / 低置信复核40个 /
...
- **⏸ 暂停点(2026-08-23 更正版)**: 主代理完成全部前置任务后即停——**B/C 方案(PRODUCTION_ROADMAP.md)由用户本人评估**,不派评估代理、不执行其任何批次(含 mingw 冒烟等 C 前置实验,已中止)。主代理职责:
    ...
    ②c6324 ENVI 引擎(1838行): 连续两次代理失败(上下文耗尽型) → 按 §12 规则转登记
      「需原 EXE 对照/人工分批移植」, 半成品线索见 git 历史; 不再自动重派
