...
## 72. Step1 清死 link_stubs 桩（全静态、零风险）
- 计算 link_stubs 定义 320，其中 core 未引用死桩 10 个 → 逐行删除。
  （FUN_1400280a8/06045c/068ca8/0628b4/0618f8/05d83c/05d87c/061a28/061a34/05b3a0）
- FUN_ 唯一 558→**548**；build/link 绿。
- 开采新台账：真是否被引用的判定须以**当前 core 全量**为准。
## 73. Step2 CRT尾桩标注
...
## 86. 062bdc(GDI+渲染) 判为硬项（≠不还原，登记待二进制/逐槽深挖）
- 需 4 未迁移 GDI+ 槽(ce08/ce10/ce28/ce30)+2 数据(a838/a34c)，且 cdb0/ce20 为 void* 槽需逐调用点强转、
  槽真签名不确定。可移植但属"hard"，与子代理 SKIP 一致，不强移以免虚构语义。
- 至此 Item(2) 可行还原池已处理完；剩余多为巨型/混淆/需二进制项（登记）。
## 87. 残留分类纠偏（重要修正）
> 先前把"剩余函数"笼统登成"待提供原PECMD.EXE"是**过度保守、不准确**。纠正后的准确三分：

- **① 已还原（~19 个）**：依赖/常量全在工程内，已干净移植（浮点/数值解析族、MBR扫描、设备映射等）。
- **② body 已存在、依赖未闭合**：decompiled/big_funcs 里**有完整 C 函数体**（如 070da8/0987ec/0bb718 为
  546/757/748 行），卡在「调用链 helper 未逐个还原」+「部分数据常量字节值未知」。
  → **不需要二进制也能继续还原**：逐层补依赖链；取值未知的数据常量(DAT_140xxxxx)显式标
    `/* TODO: 需原 .rdata 值 */`（结构可读、语义暂不确定）。
- **③ 真 decompile-failed（极少数）**：decompiled 无 body——**才真正需要原EXE二次反汇编**。
...
## 95. 全挖完目标建立(goal-f8a7def3) + 完整计划入文档(不改执行, 本轮仅记录)
...
## 98. P4 wave-3：下一批 40 helper 直移（[80:120]）
...
## 103. P4 命名应用（120 helper，含 link_stubs 同步）
...
## 106. P4 主线调整：自直移小体（w1 零代码产物→我亲移植）
- 实证：28 个中等/最深函数 agent 多波零代码（分析墙）；w1 仅留 tmp 抽取材料。
- 转向"主代理自直移小体"(16-60 行, 可全控验证)：首批 4 个(004fd4 工作集压缩+/006a4c 设备前缀判断/05daf8 格缩放/05dec4 字段取) 全绿入库。
...
## 124. ls-dirty 命名批首试回滚教训（goal 暂停期主代理操作）
- 尝试 37 名一次应用(--sync-ls, 2509 处替换)→ 冲突爆发根因: 库内长期存在【大小写双拼写】
  (如 FUN_14005B0B8/FUN_14005b0b8)各自带异型原型与调用风格, apply_rename2 变体合并后同符号多签名浮出;
  加之 ExecCmdDispatch 调用点裸 NULL/指针实参 vs 整型形参约定分裂。修复面扩散至 29 TU → 按
  「保持绿」纪律整体回滚至 6e16996。提案零损失(c5/c7 JSON 已存, c6 在途)。
...
## 127. G5/G6/G7/G8 连环批 + 管线最终形态
- **G5**(c5后40): 控件创建族+Dtor族14个; 管线v2: 孪生处置由"块删除"改为"ls定义static化"(零结构手术,
  根除孤儿块事故); 语义extern恢复优先于归一。
- **G6**(c6前61)/G7**(c7后70): 各一次全绿零修补——管线成熟度验证。
- **G8**(B2/B3/B4余13+决策表覆盖): 最小管线(不碰声明,保留各TU原生约定)+链接驱动迭代删ls孪生
  (含SplitNextToken两行签名特例)。140025f10 拒名(AppendLogMessage已被占), 维持FUN_待别名。
