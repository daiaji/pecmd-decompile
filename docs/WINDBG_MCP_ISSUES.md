...
## 【R24d 实测补记】(2026-08-27, C1 深水区 live 取证) — 三种新触发面 + 复发台账

> 本节回答"故障到底是怎么触发的"：全部序列为当日真实操作记录（同一 windbg-mcp
> 服务二进制，未做任何上游修改）。结论与 §0/A/E 一致且扩展：**根子在于 worker 的
> stop-wait 状态机对"resume 通路"的脆性**，本次新增了"连发 resume"与"快退进程
> 二次放行"两个此前未登记的具体配方。

### 触发面 T-1：bp 内嵌自动命令串（复现 §F，时序更凶）
- 序列：`bc *; bp 140034788 "du @rdx L40; .echo ====; g"; g`
  （原版 PECMD.EXE 031 取证，launch 会话）
- 结果：`g` 无任何返回 → 此后该会话一切命令 `0x80040205`（半死）→ end_session 收尸。
- 判定：断点回调里的 `g`（含复合命令串）在断点命中回调上下文中发起 resume，
  dbgeng 回调不允许同步等待事件 → 引擎状态机错位即半死。**禁止一切 bp 内嵌串。**

### 触发面 T-2：单条 execute 连发多个 resume（`g; g; g; g; g`）— 新增
- 序列：`g; g; g; g; g`（msvc 031 取证，bp FUN_14006F884 附带 EvalLoopCondition bp）
- 结果：第 2 次连发（相隔一条命令 re-get 后）报 `灾难性故障 (0x8000FFFF)`，
  其后该会话命令全 0x80040205 → end_session。
- 判定：首 `g` 命中断点后目标已 stop，紧接第二个 `g` 令状态机在"已停止"上再次
  resume → 死锁化。**每个 execute 只一条 resume；g 后必须检查输出再决定下一步。**

### 触发面 T-3：快退进程（exit<2s）launch 后二次放行 → worker 进程消失 — 新增
- 序列（两次独立复现，msvc 031/031）：`launch` → `bp pecmd_msvc!PECMD_EvalLoopCondition`
  → `g`（命中）→ 读寄存器（正常）→ 第二次 `g` → 返回
  "the engine worker process … is gone — it exited, crashed, or was terminated"。
- 判定：目标在慢速断点处理窗口内跑完 exit，worker 的 WaitForDebugEvent 循环在
  进程退出清理路径上崩溃（非半死，是进程级死亡）。对照：**同型原版 PECMD.EXE
  launch 6 次单步 g 全部成功**——差异不在目标而在时序（msvc 命中密集、回调慢）。
- 处置：worker gone 的会话直接作废（旧 session_id 不可再用），开新会话重来；
  若再次复现，切换到源码级 fprintf 探针（见 §SOP 尾部）。

### 触发面 T-4：bp 从未命中 + g 静默放行（目标自行退出）— 复核 §E
- 序列：`bp pecmd_msvc!FUN_14006F884; g`（031 内该函数实际未被调用）
- 结果：`g` 无命中返回（目标跑完退出）→ 紧接着的任何命令 `0x80040205`。
- 判定：bp 目标在本场景根本不会被调（031 的 032dc4 分派走 01E69C）——**bp 未命中
  不等于"没跑到"，先想清楚函数是否必达；确认必达仍不中再怀疑 bp 地址**（§R19 教训）。

### 复发台账（当日全部，附带处置耗时）

| 会话 | 触发面 | 首次症状 | 处置 |
|---|---|---|---|
| 原版031 命令串bp | T-1 | g 无返回 → 全 0x80040205 | end_session ~10s |
| 原版031 06F884-bp | T-4 | g 无命中 → .echo 即 0x80040205 | end_session |
| 原版031 06F884-bp② | T-4 | 同上 | end_session |
| msvc031 连发g | T-2 | 0x8000FFFF | end_session |
| msvc031 launch | T-3 | 二次 g → worker gone | 弃会话重开 |
| msvc031 launch② | T-3 | 同上 | 弃会话重开 → 改探针 |

对照组（当日全程稳定）：原版 launch + **单条 g + 立即读寄存器** ×6g（031 标志取证
全部成功）；R24 子代理 D 三案 attach 挂死进程（k/du/read_memory 全套）零故障。

### 机制总结（一句话版）

> worker 把"能 resume/等待目标事件"的命令（g/bp内嵌串/连发g）与"查询类"命令
> 混在同一请求队列时，若目标在断点回调或快速退出窗口内移动，worker 状态机
> 丢失 stop-wait 一致性 → 半死（0x80040205）/灾难（0x8000FFFF）/进程消亡三种终态。
> **规避 = 单查单放：一条 execute 只放行一次并立即检查；密集取证优先探针。**

---

## 【故障浮现速查卡】(遇 windbg MCP 异常先看这里)

| 症状（报错原文） | 判定 | 立即处置 |
|---|---|---|
| `0x80040205` 提出的一个意外的意外现象 | 会话半死（不可恢复） | 立即 `end_session`；复用同一会话=浪费轮次 |
| `0x8000FFFF` 灾难性故障 | 连发 resume 或 bp 内嵌串所致 | 同上；后续拆单条 g |
| `engine worker process is gone` | worker 进程级崩溃 | 弃旧 session_id 重开；复发则切探针 |
| 命令正常但 bp 从不命中 | 目标可能没走到该函数（先静态确认必达性） | 不赖 bp；改源码级 fprintf 探针验证 |
| attach 挂死进程一切正常 | 健康路径（推荐） | 继续；挂死/慢进程一律 attach 优先 |

预防三律：① execute 每条只含一条 resume（g/p/t），放行后先看输出；② 禁 bp 内嵌
自动命令串；③ 取证目标为"必达热点/快退进程"时直接上 fprintf 探针（一次构建全量数据）。
