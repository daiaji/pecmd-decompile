...
> **版本：v4.2（2026-08-27 修订记录）｜v4（2026-08-24 用户方向裁定后重写）｜上一版：v3（场景 B/C 框架，已废弃）**
...
> ---
> **v4.2 修订（2026-08-27，对齐 S14 条款）**：现代化裁定以 §S14 为准（08-25 生效，
> 取代 08-24"不要现代化"非目标口径）。对"现代化"按可读性增益与风险分三层管理：
> **L1 语义化**（local_xx/param_N→语义名、注释、文档，纯命名零行为风险）——
> 提前并行执行，依据 P3/B1 与「与验证循环并行」条款，按"语料/扫描表已覆盖的真值路径
> 优先"逐函数推进，不等待 S14 门链；
> **L2 结构重构**（巨体按动词分臂拆解、goto 状态机→结构化控制流）——归 S14，
> 其安全前置新增两条：① 夹具 vars 值观测升级（U-2，exit 单维盲区补齐）；
> ② 巨体输入扫描基线（原版直跑建"输入→行为"表，如 R24f bf358 同法）。
> **L3 架构级现代化**（全树重写/风格门禁/C23 迁移）——仍非目标（证据链/dc 行号锚点
> 优先，Lua FFI 对照北极星要求对照面稳定）。
...
### S14 里程碑：结构化重写与现代化（2026-08-25 用户裁定立项，取代 v4 非目标条款）
> **用户新裁定（覆盖 08-24"不要现代化"非目标）**：最终目标=人类可读+人类编码风格+
> 可编译+功能对齐原始实现。允许适当必要重写；初步还原完成后，对理解代码有益的
> 大模块拆分/重写/代码现代化**可以做**。

### 定位与铁律
- 前提链：门A ✅ → **T4 全量收敛（PASS + divergences 定稿）** → S12 头文件统一
  → S13 命名清零 → **S14 启动**。
- 每次重写的行为中性证明 = 重写前后 28 语料全 PASS + golden diff 无新增差异；
  有意偏差必须登记 divergences.md。
- 优先级=人类阅读热度：主循环/EXEC/ENVI/WRITE 核心路径先行；巨函数(ECD/PSB)
  按 verb 分臂拆分为独立函数；冷区长尾保持 L1 即可并注"未重构区"。

### 波次草案
| 波次 | 内容 |
|---|---|
| S14-a | FourCC→可读常量(FOURCC宏)、消除双重强转噪音、魔数具名化 |
| S14-b | PSB/ECD 巨函数按动词分臂拆分（每臂独立函数+语义命名） |
| S14-c | goto 状态机→结构化控制流（仅限可证明等价的区段） |
| S14-d | 函数级行为文档补全（现有"直移"注释升级行为说明） |

### 北极星补记（2026-08-25 用户澄清）
终态用例 = 本参考库作为 **Lua FFI 重写 PECMD 的对照基线**。
- 阶段一（当前）：117 动词功能完全对齐——以 s12_verb_coverage_matrix.md 为主驱动，
  每动词有语料 PASS 证据；R1 类双体分裂与占位桩清零是覆盖前置。
- 阶段二：S14 模块化可读性重构（巨函数按动词拆臂 ↔ 未来 Lua 模块划分天然对应）。
