...
typedef union _LARGE_INTEGER { struct { DWORD LowPart; LONG HighPart; } s; struct { DWORD LowPart; LONG HighPart; } u; long long QuadPart; } LARGE_INTEGER;
...
typedef union FTU64 { uint64_t v; FILETIME ft; } FTU64;
...
extern BOOL PECMD_CreateProcessAsUser(LPCWSTR, LPWSTR, LPSECURITY_ATTRIBUTES, LPSECURITY_ATTRIBUTES,
        ...
        uint32_t, LPCWSTR, LPCWSTR, LPCWSTR);
...
extern uint64_t FreeLibrary(void *h);
                                        /* 既存声明@7394 前移 */
extern uint8_t *PECMD_LoadEncodedResource(void *a, uint16_t *b, uint16_t *c, int64_t *d, unsigned int *e);
 /* def 本文件@3823 */
...
extern uint64_t PECMD_GenerateTimeText(LPCWSTR p1, int64_t *p2, uint64_t p3, uint64_t p4, uint64_t p5);
...
extern int PECMD_ReadRegBinaryGuarded(HKEY hkey, LPCWSTR path, LPCWSTR name,
        longlong *buf, LPDWORD size, long *extra);
...
extern LARGE_INTEGER PECMD_ProcessScriptBlock(LARGE_INTEGER p1, LARGE_INTEGER p2,
        longlong *p3, longlong *p4, pthreadmbcinfo p5);
                      /* def 本文件@6341 */
...
extern LPCWSTR PECMD_ConvertLfToCrlf(LPCWSTR a, int64_t *b);
                                     /* core_b3_remaining.c */
extern int64_t PECMD_DispatchByObjectName(uint64_t *, LPCWSTR, int64_t, uint16_t *, uint64_t, uint64_t);
 /* core_b3_remaining.c */
extern void PECMD_VarWriteLine(void *script, LPCWSTR key, LPCWSTR data, int64_t len, int64_t *pkey, char mode);
 /* core_remaining_helpers.c */
...
int      wsprintfW(unsigned short *out, const unsigned short *fmt, ...);
   /* 定义见后部 (def @3623) */
int      FUN_140003864(void *a, const uint16_t *b, uint32_t c, uint32_t d, void *e, uint32_t f, uint32_t g, void *h);
...
DWORD     FUN_14006459C(LPCWSTR src, uint32_t buflen, LPWSTR buf, LPWSTR *last);
 /* core_exec2.c */
int64_t   PECMD_DescribePartitionInfo(int64_t *param_1, LPCWSTR param_2, uint64_t param_3, uint64_t param_4, LARGE_INTEGER *param_5, uint32_t *param_6, char param_7);
 /* core_b3_remaining.c */
...
extern uint64_t  PECMD_RegiEditRegistry(uint64_t, uint64_t, uint64_t);
               /* @0x1400c13f8 (后部定义) */
extern HANDLE    PECMD_CreateDetachedThread(LPTHREAD_START_ROUTINE, LPVOID, size_t, DWORD, LPDWORD, LPSECURITY_ATTRIBUTES);
  /* @0x14005b228 (后部定义) */
...
uint64_t PECMD_ScanHexNumber(uint16_t **ps, uint64_t *acc, int *max);
      /* def core_b3_remaining.c */
int64_t  PECMD_ReadModifyWrite(HANDLE param_1, uint8_t *param_2, int64_t param_3, int param_4, uint32_t param_5);
  /* def core_b3r_c.c */
bool     PECMD_WriteFileChunked(HANDLE hFile, const void *data, uint32_t len, int *written, uint32_t minChunk);
   /* def core_b3_remaining.c */
void     FUN_14006355c(void *a, const WCHAR *b, int c, uint64_t d);
...
extern int64_t  PECMD_TreeCollectCheckedText(int64_t obj, int64_t *out, uint64_t hItem, int64_t mode);
...
extern int64_t  PECMD_TreeScanCheckedSiblings(int64_t obj, int64_t *out, uint64_t hItem, uint64_t flags);
...
uint64_t AssignProcessToJobObject(void *job, void *proc);
uint64_t Beep(void);
...
HANDLE CreateFileMappingW(HANDLE h, LPSECURITY_ATTRIBUTES sa, DWORD prot, DWORD maxHi, DWORD maxLo, LPCWSTR name);
...
uint64_t CreateProcessW(LPCWSTR app, const uint16_t *cmd, LPSECURITY_ATTRIBUTES sa, LPSECURITY_ATTRIBUTES ta, BOOL inh, DWORD flags, void *env, LPCWSTR dir, void *si, void *pi);
uint64_t CreateRoundRectRgn(void);
void *CreateServiceW(void *mgr, const unsigned short *name, const unsigned short *disp, unsigned long acc, unsigned long type, unsigned long start, unsigned long err, const unsigned short *bin, const unsigned short *grp, unsigned long *tag, const unsigned short *deps, const unsigned short *acct, const unsigned short *pwd);
HBRUSH CreateSolidBrush(COLORREF color);
void *CreateThread(void *sa, size_t st, void *rt, void *p, uint32_t f, uint32_t *tid);
uint64_t CreateToolhelp32Snapshot(DWORD flags, DWORD pid);
HWND CreateWindowExW(DWORD ex, const WCHAR *cls, const WCHAR *name, DWORD style, int x, int y, int w, int h, HWND parent, HMENU menu, void *inst, void *param);
...
int DestroyWindow(void *h);
int DeviceIoControl(void *h, unsigned long code, void *in, unsigned long inb, void *out, unsigned long outb, unsigned long *ret, void *ov);
...
uint64_t DuplicateHandle(void *hSrcP, void *hSrc, void *hDstP, HANDLE *phDst, DWORD acc, BOOL inh, DWORD opts);
...
HANDLE CreateNamedPipeA(LPCSTR name, DWORD openMode, DWORD pipeMode, DWORD maxInst, DWORD outSz, DWORD inSz, DWORD timeout, LPSECURITY_ATTRIBUTES sa);
...
uint64_t PECMD_WaitHandlesOrMessages(uint64_t param_1, int64_t param_2, int param_3, uint64_t *param_4);
...
uint64_t PECMD_AddVarDefault(void *script, LPCWSTR name, LPCWSTR val, int len, int64_t flag);
int64_t PECMD_FindVarValue(int64_t *a, LPCWSTR b, int64_t *c, int d);
...
uint16_t *PECMD_NextToken(int64_t *a, int64_t *b, uint32_t c);
void PECMD_AutoMountStartup(long long *param_1, LPCWSTR param_2);
uint32_t FormatMessageW(uint32_t flags, const void *src, uint32_t msg, uint32_t lang, WCHAR *buf, uint32_t size, void *args);
void FUN_14001d78c(unsigned char *dst, const unsigned char *src, longlong n);
void FUN_140025f10(longlong param_1,LPCWSTR param_2,uint32_t param_3,char *param_4, char *param_5,long long *param_6);
uint64_t PECMD_ScriptInitParse(uint64_t a, uint64_t b, uint64_t c);
undefined8 *FUN_14001877C(longlong *ps, int count);
COORD GetLargestConsoleWindowSize(void *h);
int SetConsoleWindowInfo(void *h, int b, void *r);
uint64_t PECMD_ExpandSpecialDirs(void *param_1, const uint16_t *param_2, longlong param_3, longlong param_4, const uint16_t *param_5);
...
LARGE_INTEGER PECMD_ProcessScriptBlock(LARGE_INTEGER param_1,LARGE_INTEGER param_2,longlong *param_3,longlong *param_4, pthreadmbcinfo param_5);
uint64_t PECMD_IsPecmdScriptFile(uint64_t a);
uint64_t PECMD_UserCmdHandler(uint64_t a);
uint64_t PECMD_MstrStringOps(uint64_t a, uint64_t b);
...
HANDLE PECMD_CreateDetachedThread(LPTHREAD_START_ROUTINE param_1, LPVOID param_2, size_t param_3, DWORD param_4, LPDWORD param_5, LPSECURITY_ATTRIBUTES param_6);
...
DWORD PECMD_RegDeleteValue(HKEY root, LPCWSTR subkey, LPCWSTR name);
longlong PECMD_TokPrefixICmp(char *param_1, ushort *param_2, int param_3);
int PECMD_AsciiPrefixICmp(const char *a, const uint16_t *w, int n);
...
uint64_t PECMD_NextRandomSeed(void);
...
uint16_t *PECMD_AllocStrSlot(uint16_t **out);
longlong *PECMD_AppendWideStr(void *param_1p,LPCWSTR param_2);
WCHAR *PECMD_StrDupA(WCHAR **ps, LPCWSTR src, int64_t a, int64_t b);
...
long long FUN_140064a34(uint16_t *s);
uint64_t PECMD_ParseIntegerString(long long *param_1, uint64_t *param_2);
int PECMD_ParseNumTryWriteback(long long *param_1, int *param_2);
...
void PECMD_HeapFreeWithHeader(long long *a);
...
uint64_t PECMD_ResDecode(int64_t *ps, uint32_t flag);
...
uint64_t FreeEnvironmentStringsW(LPWSTR env);
uint64_t FreeLibrary(void *h);
...
DWORD GetCurrentDirectoryW(DWORD n, WCHAR *buf);
uint64_t GetCurrentProcess(void);
...
uint64_t GetEnvironmentStringsW(void);
uint64_t GetEnvironmentVariableA(void);
DWORD GetEnvironmentVariableW(const WCHAR *n, WCHAR *buf, DWORD sz);
uint64_t GetExitCodeProcess(void *h, DWORD *code);
...
unsigned long GetModuleFileNameW(void *h, WCHAR *buf, unsigned long n);
void *GetModuleHandleA(const char *m);
HMODULE GetModuleHandleW(const WCHAR *m);
...
uint64_t GetProcessHeap(void);
uint64_t GetProcessTimes(HANDLE h, FILETIME *a, FILETIME *b, FILETIME *c, FILETIME *d);
...
int HeapFree(void *h, DWORD f, void *p);
...
uint64_t MsgWaitForMultipleObjects(DWORD count, HANDLE *handles, BOOL waitAll, DWORD timeout, DWORD mask);
...
uint64_t OpenProcess(DWORD access, BOOL inherit, DWORD pid);
...
uint64_t PathMatchSpecW(const uint16_t *a, const uint16_t *b);
uint64_t PathFileExistsW(void);
int PeekMessageW(void *msg, void *hwnd, UINT a, UINT b, UINT c);
uint64_t Polyline(void);
int PostMessageW(void *w, unsigned int m, uint64_t a, uint64_t b);
uint64_t PostQuitMessage(void);
uint64_t Process32FirstW(HANDLE h, PROCESSENTRY32W *pe);
uint64_t Process32NextW(HANDLE h, PROCESSENTRY32W *pe);
int PtInRect(const void *r, POINT p);
uint64_t QueryDosDeviceW(void);
uint64_t SetVolumeMountPointW(void);
uint64_t GetVolumeNameForVolumeMountPointW(void *mount, void *buf, unsigned long n);
uint64_t QueryPerformanceCounter(void *out);
...
int RegCloseKey(void *k);
int RegCreateKeyExW(void *k, const WCHAR *sub, DWORD res, void *cls, DWORD opt, DWORD acc, void *sa, void *out, DWORD *disp);
...
int RegQueryValueExW(void *k, const WCHAR *nm, DWORD *rs, DWORD *ty, BYTE *dt, DWORD *sz);
...
uint64_t ResumeThread(void *hThread);
uint64_t SHChangeNotify(void);
uint64_t SHFileOperationW(uint64_t a);
uint64_t SHGetSpecialFolderPathW(HWND hwnd, WCHAR *path, int csidl, BOOL fCreate);
...
intptr_t SendMessageW(void *h, UINT m, uint64_t w, uint64_t l);
uint64_t SetActiveWindow(void *h);
...
int SetEnvironmentVariableW(const WCHAR *n, const WCHAR *v);
int SetEvent(void *h);
uint64_t SetFilePointer(void *f, long dist, LONG *distHi, DWORD how);
uint64_t SetFilePointerEx(void *h, longlong off, longlong *out, unsigned long method);
...
uint64_t SetProcessWorkingSetSize(void *h, uint64_t a, uint64_t b);
...
int SetWindowPos(void *w, void *after, int x, int y, int cx, int cy, unsigned int f);
int SetWindowRgn(HWND h, void *r, BOOL b);
int SetWindowTextW(void *hWnd, const void *lpString);
...
LPWSTR StrChrW(const WCHAR *s, WCHAR c);
uint64_t StrCmpNIA(const char *a, const char *b, int n);
int StrCmpNIW();
void *OpenDesktopW(const WCHAR *n, uint64_t f, uint64_t acc, uint64_t flags);
int SetThreadDesktop(void *d);
int SwitchDesktop(void *d);
...
LPWSTR StrRChrW(const WCHAR *start, const WCHAR *end, WCHAR c);
...
uint64_t TerminateJobObject(void *h, uint32_t code);
uint64_t TerminateProcess(void *h, uint32_t code);
uint64_t TerminateThread(void);
uint64_t TrackPopupMenu(void *menu, uint32_t flags, int x, int y, uint32_t res, void *wnd, void *rect);
int TranslateMessage(const void *m);
...
uint64_t WaitForSingleObject(void *h, uint64_t ms);
...
uint64_t wsprintfA(char *out, const char *fmt, ...);
int wsprintfW(unsigned short *out, const unsigned short *fmt, ...);
...
int32_t PECMD_DecodeEncTextToUtf16(uint32_t spec, const uint8_t *src, int srclen, uint16_t *dst, int dstcap, uint32_t key);
...
void PECMD_CreateStaticControlFromRect(int64_t *a, const void *b, unsigned int c, uint64_t *d, int64_t e, unsigned int f);
...
uint16_t PECMD_DispatchMessageMap(int64_t *param_1, int param_2, uint64_t param_3, uint64_t *param_4, int64_t param_5, uint8_t param_6, uint64_t *param_7);
...
uint64_t PECMD_ParseControlMessage(int64_t *a, uintptr_t b, uintptr_t c, const uint16_t *d, uintptr_t e, int64_t f);
uint64_t PECMD_ControlEnableCommand(int64_t *a, uintptr_t b, uintptr_t c, const uint16_t *d, uintptr_t e, int64_t f);
...
uint64_t CreateProcessAsUserW(void *tok, LPCWSTR app, WCHAR *cmd, LPSECURITY_ATTRIBUTES sa, LPSECURITY_ATTRIBUTES ta, BOOL inh, DWORD flags, void *env, LPCWSTR dir, void *si, void *pi);
uint64_t CreateProcessWithLogonW(void *user, void *domain, void *pwd, DWORD flags, void *app, WCHAR *cmd, DWORD flg2, void *env, void *dir, void *si, void *pi);
...
void FUN_140021144(const WCHAR *p);
void FUN_140019da8(long long p1, const WCHAR *p2, int p3);
void FUN_140020fd4(long long p1, char c, const WCHAR *p2, const WCHAR *p3);
uint PECMD_InstallPlugPlayDriver(long long param_1, LPCWSTR param_2, LPCWSTR param_3, uint param_4, int param_5, uint8_t *param_6);
...
uint64_t GetShortPathNameW(LPCWSTR path, WCHAR *buf, DWORD len);
uint64_t CreateJobObjectW(LPSECURITY_ATTRIBUTES sa, LPCWSTR name);
...
uint64_t WaitForInputIdle(HANDLE proc, DWORD ms);
...
long long FUN_14006b8fc(long long *param_1);
void PECMD_InitButtonState(long long param_1, uint param_2, uint64_t param_3);
bool PECMD_CreateButtonSubCtl(long long *param_1, LPCWSTR param_2, uint64_t param_3, int *param_4, HWND param_5, uint param_6, DWORD param_7);
...
uint64_t FUN_1400662a4(int64_t *p1, void *p2, int64_t *p3, const uint16_t *p4, const uint16_t *p5, uint p6);
...
int64_t FUN_140065864(int64_t param_1, int64_t *param_2, int64_t *param_3, uint8_t *param_4, uint32_t param_5);
...
void *FUN_1400aa144(void *a, longlong b, int c, void *d, int e, int f, int g, int h, uint32_t i, WCHAR *j, uint8_t k);
void *FUN_1400b8f10(void *a, longlong b, int c, void *d, int e, int f, int g, int h, void *i, uint32_t j, int *k, const WCHAR *l);
void *FUN_1400bca60(void *a, longlong b, int c, void *d, int e, int f, int g, int h, void *i, void *j, uint32_t *k, uint32_t l, uint32_t m, uint32_t n);
void *RegisterServiceCtrlHandlerW(const WCHAR *name, void *handler);
int SHGetSpecialFolderPathA(void *hwnd, char *buf, int folder, int create);
char *lstrcatA(char *dst, const char *src);
uint FUN_14001b608(uint64_t a);
long long FUN_140031454(long long *a, pthreadmbcinfo b);
int FUN_140067cf4(long long *a, uint64_t *b);
void PECMD_ExecInterpString(uint64_t a, void *b);
uint64_t FUN_14004fb44(longlong *a, ulonglong b);
void FUN_14005d9a8(int64_t a, int b);
uint64_t FUN_1400630d0(int a);
...
void FUN_1400702b0(uint16_t **ps, const uint16_t *src);
int64_t *FUN_14007034c(int64_t *param_1, const uint16_t *param_2);
LPCWSTR PECMD_LoadLocalizedString(void *hinst, unsigned int id, WCHAR *buf, int len);
longlong FUN_140003a20(longlong *param_1, undefined8 *param_2, byte param_3);
undefined8 FUN_1400748a0(HWND param_1, undefined8 *param_2);
...
void *FUN_1400b3d0c(const unsigned short *a, longlong *b, longlong c, longlong d, unsigned int *e, undefined8 *f, int g);
...
void PECMD_MenuRegisterCommand(byte *param_1,ushort *param_2,HMENU param_3,longlong param_4,longlong param_5, longlong *param_6);
undefined8 FUN_14005c788(char *a, ushort *b, int c);
...
int PECMD_ReadRegBinaryGuarded(HKEY param_1,LPCWSTR param_2,LPCWSTR param_3,longlong *param_4,LPDWORD param_5, LSTATUS *param_6);
...
longlong *FUN_1400637dc(longlong *a, LPCSTR b, ulonglong c, ulonglong d);
...
void FUN_1400629b8(void *a, const WCHAR *b, const WCHAR *c);
...
undefined8 FUN_1400f35b8(HICON ic, longlong *pw, longlong *ph);
HICON FUN_14001f1d4(LPCWSTR path, ulonglong *extra);
undefined4 FUN_1400ede48(HDC dc, HICON ic, undefined8 a3, int a4, ulonglong a5, ulonglong a6, undefined4 a7, undefined4 a8, ulonglong a9, ulonglong a10, COLORREF a11);
void FUN_140066eac(longlong a1, int a2);
ulonglong FUN_140082520(longlong *a1, WCHAR *a2, longlong a3, int a4);
