...
extern int64_t PECMD_SrParsePrefix(void *script, WCHAR **pp, int *flags, int *flags2,
                                   ...
                                   WCHAR **p_outbuf, int *p_qkmode); /* @0x140031454 段1 */
...
int64_t PECMD_RunCommand(void *script, WCHAR *cmdline)
{
    ...
    /* ---- 段1：前缀指令解析 ---- */
    PECMD_SrParsePrefix(script, &pp, &flags, &flags2, &m_flag, &mem_flag,
                        &sysinit_name, &b_sysinit, &outbuf, &qkmode);
    ...
    /* ---- 主展开路径（LAB_140031887）---- */
    {
        ...
        PECMD_AllocWStringBuffer((WCHAR **)&local_250, 0);   /* 反编译 PECMD_AllocStrSlot(&local_278)? 实为 local_250 构造 */
        ...
        if (mapStr != NULL) {
            ...
            if (PECMD_ParseUIntValue(&mapStr, (int64_t *)&mapOff) > 0) {
                ...
                if (mv != NULL) {
                    ...
                    UnmapViewOfFile(mv);
                    if (sz > 0) {
                        void *mv2 = MapViewOfFile((HANDLE)(intptr_t)mapOff, 6, 0, 0,
                                                  (size_t)(sz + 2));
                        if (mv2 != NULL) {
                            local_1a0 = (WCHAR *)mv2;
        ...
        /* ---- 资源脚本执行路径（反编译 391-752）---- */
        {
            ...
            if (resName[0] != L'\0' && resName[1] == L'"') resName[1] = 0;
        ...
        (void)local_228; (void)local_1f0; (void)local_234; (void)local_26c;
        (void)lVar12;
        return (int)DVar13;
    }
}
